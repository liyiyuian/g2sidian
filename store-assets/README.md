# G2sidian — store assets

Copy-and-paste materials for the Even Hub listing.

- **listing.txt** — full listing: name, tagline, short + long description, category, permission
  justifications, privacy summary, terms.
- **tester-quickstart.txt** — ≤1000 chars; doubles as the store **Description** for beta/tester builds.
- **privacy.txt** — privacy policy (host it somewhere linkable; fill in `<your contact email>`).
- **screenshots/** — 576×288 glasses captures, **rendered from a fake demo vault ("Notebook"),
  not real notes**:
  - `1-browser.png` — vault home: folder tree + Quick capture + Search rows
  - `2-reader.png` — a note rendered to clean lines (heading, bullets, wikilink, tags)
  - `3-tasks.png` — a `tasks` query block rendered as live "due today & overdue" results
  - `4-capture.png` — hands-free voice quick-capture to today's daily note

Screenshots were produced by pointing the backend at a demo vault and driving the Even Hub
simulator — **never the owner's real vault**. App icon is uploaded portal-side (no manifest field).
